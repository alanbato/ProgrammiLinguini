...
firstn(_, 0) -> [];
firstn([First|Rest], N) -> [First|firstn(Rest, N-1)].

% === lastn ===

lastn(Lst, 0) -> Lst;
lastn([_|Rest], N) -> lastn(Rest, N-1).

% === sum ===

sum([]) -> 0;
sum([First|Rest]) -> sum(First) + sum(Rest);
sum(First) -> First.
% === even ===
...
reverse([]) -> [];
reverse([First|Rest]) -> if 
	is_list(First) -> reverse(Rest) ++ [reverse(First)];
	true -> reverse(Rest) ++ [First]
end.
...
maskedSum([], []) -> 0;
maskedSum([Fval|Rval], [Fmask|Rmask]) -> if
	Fmask -> Fval + maskedSum(Rval, Rmask);
	true -> maskedSum(Rval, Rmask)
end.

% === evaluate ===

evaluate([], _) -> 0;
evaluate([First], _) -> First;
evaluate([First|Rest], N) -> First * math:pow(N,length([First|Rest])-1) + evaluate(Rest, N).
