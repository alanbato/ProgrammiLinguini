...
add (Complex a bi) (Complex c di) = (Complex(a+bi) (c+di))

multiply :: Complex -> Int -> Complex
multiply (Complex a bi) x = (Complex(a*x) (bi))

getReal :: Complex -> Int
getReal (Complex a _) = a

getImaginary :: Complex -> Int
getImaginary (Complex _ bi) = bi
...
getMaxComponent (RGB (r, g, b)) = if (max (max r g) b) == r
                                  then ("Red", r)
